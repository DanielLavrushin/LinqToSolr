    ...
    public static class JsonParser
    {
        ...
        internal static object ParseValue(Type type, string json)
        {
...
#if NET40_OR_GREATER
                Guid result;
                Guid.TryParse(json.Replace("\"", ""), out result);
                return result;
#else
            ...
            if (type.GetTypeInfo().IsGenericType && type.GetGenericTypeDefinition() == typeof(ICollection<>))
            {
                ...
                List<string> elems = Split(json);
                var listGenericType = typeof(List<>).MakeGenericType(listType);
        ...
        static Dictionary<string, T> CreateMemberNameDictionary<T>(T[] members) where T : MemberInfo
        {
            ...
            {
                T member = members[i];
                if (member.IsDefined(typeof(SolrFieldIgnoreAttribute), true))
                    continue;
                ...
                {
#if NET40 || NET35
                    var dataMemberAttribute = Attribute.GetCustomAttribute(member, typeof(SolrFieldAttribute), true) as SolrFieldAttribute;
#else
        ...
        static object ParseObject(Type type, string json)
        {
            ...
            {
                nameToField = CreateMemberNameDictionary(type.GetFields(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.FlattenHierarchy));
                fieldInfoCache.Add(type, nameToField);
            }

            if (!propertyInfoCache.TryGetValue(type, out nameToProperty))
            {
                nameToProperty = CreateMemberNameDictionary(type.GetProperties(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.FlattenHierarchy));
